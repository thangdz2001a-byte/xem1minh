    ...
    <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex justify-center pt-16 md:pt-24 px-4">
      ...
      <div className="relative w-full max-w-3xl bg-[#111] rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[75vh] overflow-hidden">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (query) { setView({ type: "search", keyword: query }); onClose(); }
          }}
        ...
        <div className="overflow-y-auto flex-1 p-2 no-scrollbar">
          {loading ? (
            ...
            <>
              {results.map((m) => <SearchItem key={m.slug} m={m} setView={setView} onClose={onClose} />)}
              <button
...
const MovieSection = ({ title, slug, type = "the-loai", setView, progressData }) => {
  ...
  useEffect(() => {
    const controller = new AbortController();
    fetch(`${API}/${type}/${slug}`, { signal: controller.signal })
      .then((r) => r.json())
    ...
    <div className="mb-8 md:mb-12 relative group/section">
      <div className="flex items-center justify-between mb-3 md:mb-4 px-1">
        <h2 className="text-[15px] sm:text-lg md:text-2xl font-black text-white uppercase tracking-tighter flex items-center gap-2 md:gap-3">
          <span className="w-1 md:w-1.5 h-4 sm:h-5 md:h-7 bg-[#E50914] rounded-full" /> {title}
        </h2>
        <button onClick={() => setView({ type: "list", slug, title, mode: type })} className="text-[#E50914] text-[9px] sm:text-[10px] md:text-xs font-black hover:underline opacity-100 md:opacity-0 group-hover/section:opacity-100 transition-opacity uppercase tracking-widest">Xem tất cả</button>
      </div>
      <div className="relative">
        ...
        <div ref={scrollRef} className="flex gap-3 sm:gap-4 md:gap-6 overflow-x-auto scroll-smooth no-scrollbar pb-4 px-1 md:px-2">
          {movies.map((m) => <MovieCard key={m.slug} m={m} setView={setView} progressData={progressData} isRow={true} />)}
        </div>
  ...
  return (
    <div className="mb-8 md:mb-12 animate-in slide-in-from-left duration-500 group/section">
      <div className="flex items-center mb-3 md:mb-4 px-1">
        <h2 className="text-[15px] sm:text-lg md:text-2xl font-black text-white uppercase tracking-tighter flex items-center gap-2 md:gap-3">
          <span className="w-1 md:w-1.5 h-4 sm:h-5 md:h-7 bg-[#E50914] rounded-full" /> Tiếp tục xem
        </h2>
      </div>
      <div className="relative">
        <button onClick={() => scroll("left")} className="absolute left-0 top-[40%] -translate-y-1/2 z-[40] bg-black/60 hover:bg-black/90 text-white p-2 md:p-3 rounded-r-2xl backdrop-blur-md opacity-0 group-hover/section:opacity-100 transition-all hidden md:flex items-center shadow-2xl">
              ...
              <div className="relative group cursor-pointer flex items-center gap-1 hover:text-white transition uppercase">
                ...
                <div className="absolute hidden group-hover:grid grid-cols-3 gap-3 bg-[#111]/95 backdrop-blur-xl p-6 w-[500px] rounded-2xl top-full left-0 border border-white/10 shadow-2xl mt-4">
                  {categories && categories.map((c) => (
                    <button key={c.slug} onClick={() => setView({ type: "list", slug: c.slug, title: c.name, mode: "the-loai" })} className="text-left text-xs font-bold text-gray-400 hover:text-white hover:pl-2 transition-all uppercase tracking-tight">{c.name}</button>
                  ))}
                </div>
              </div>
            </nav>
      ...
      <div className={`md:hidden fixed inset-0 bg-black/90 z-[110] backdrop-blur-sm transition-opacity duration-300 ${showCat ? "opacity-100" : "opacity-0 pointer-events-none"}`} onClick={() => setShowCat(false)}>
        <div className={`absolute bottom-0 w-full bg-[#111] rounded-t-3xl p-6 transition-transform duration-500 delay-100 ${showCat ? "translate-y-0" : "translate-y-full"}`}>
          ...
          <div className="grid grid-cols-2 gap-3 overflow-y-auto max-h-[50vh] pb-8">
            {categories && categories.map((c) => (
              <button key={c.slug} onClick={() => { setView({ type: "list", slug: c.slug, title: c.name, mode: "the-loai" }); setShowCat(false); }} className="bg-white/5 py-3 rounded-xl text-xs text-gray-300 font-bold active:bg-[#E50914] transition uppercase tracking-tight">{c.name}</button>
            ))}
          </div>
        </div>
      </div>
...
const Hero = ({ setView }) => {
  ...
  if (loading) return <div className="w-full h-[60vh] sm:h-[70vh] md:h-[85vh] flex justify-center items-center bg-[#050505]"><Icon.Loader2 className="animate-spin text-[#E50914]" size={36} /></div>;
  if (bannerMovies.length === 0) return null;

  return (
    <div className="relative w-full h-[60vh] sm:h-[70vh] md:h-[85vh] bg-[#050505] overflow-hidden">
      {bannerMovies.map((movie, index) => <HeroSlide key={movie.slug} movie={movie} isActive={index === currentIndex} setView={setView} />)}
      <div className="absolute bottom-4 sm:bottom-6 md:bottom-8 left-0 w-full flex justify-center items-center gap-1.5 md:gap-2 z-20">
        {bannerMovies.map((_, i) => (
          <button key={i} onClick={() => setCurrentIndex(i)} className={`rounded-full transition-all duration-500 ${i === currentIndex ? "w-5 h-1 md:w-8 md:h-1.5 bg-[#E50914]" : "w-1 h-1 md:w-1.5 md:h-1.5 bg-gray-500 hover:bg-gray-300"}`} />
        ))}
      </div>
    </div>
  );
      ...
      <div className="w-full px-4 md:px-12 lg:px-16 mt-8 md:mt-12 grid md:grid-cols-12 gap-6 md:gap-8 items-start">
        <div className="md:col-span-8 bg-[#111] p-5 md:p-8 rounded-2xl border border-white/5 shadow-xl h-fit">
          <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
            ...
            <h3 className="text-sm md:text-base font-black text-white uppercase tracking-widest">Nội dung phim</h3>
          </div>
          <div className="text-gray-300 text-[14px] md:text-lg leading-relaxed font-medium" dangerouslySetInnerHTML={{ __html: i?.content }} />
        </div>
        <div className="md:col-span-4 bg-[#111] p-5 md:p-8 rounded-2xl border border-white/5 shadow-xl space-y-6 md:space-y-8 h-fit">
          {[{ l: "Quốc gia", v: i?.country?.map((c) => c.name).join(", ") }, { l: "Thể loại", v: i?.category?.map((c) => c.name).join(", ") }, { l: "Diễn viên", v: i?.actor?.slice(0, 8).join(", ") }].map((x) => (
            <div key={x.l} className="space-y-1.5 md:space-y-2 border-b border-white/5 pb-4 md:pb-6 last:border-0 last:pb-0">
