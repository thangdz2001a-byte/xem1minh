...
const SmartImage = ({
  ...
  if (data) {
    const tmdbPath =
      type === "backdrop" ? data.backdrop_path : data.poster_path;
    if (tmdbPath) {
      finalSrc = `https://image.tmdb.org/t/p/${
        type === "backdrop" ? "original" : "w500"
      }${tmdbPath}`;
    }
  }
  ...
  return (
    <img
      src={
        loading
          ? "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 2 3'%3E%3C/svg%3E"
          : finalSrc
      }
      alt={alt}
      className={className}
...
const SearchModal = ({ isOpen, onClose, setView }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex justify-center pt-16 md:pt-24 px-4">
      ...
      <div className="relative w-full max-w-3xl bg-[#111] rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[70vh] overflow-hidden">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (query) {
          ...
          className="flex items-center p-4 border-b border-white/5 bg-[#1a1a1a]"
        >
          ...
          <input
            ...
            className="w-full bg-transparent outline-none text-white pl-10 pr-10 py-2 text-base md:text-lg"
          />
        </form>
    ...
    return (
      <div
        className={`group/card cursor-pointer flex flex-col shrink-0 relative ${
          isRow ? "w-[120px] sm:w-[150px] md:w-56 lg:w-64" : ""
        }`}
        onClick={() => {
      ...
      >
        {onRemove && (
          <button
            ...
            className="absolute top-1.5 right-1.5 md:top-2 md:right-2 z-30 bg-black/60 hover:bg-[#E50914] text-white p-1 md:p-1.5 rounded-full backdrop-blur-md opacity-0 group-hover/card:opacity-100 transition-all border border-white/10"
          >
            <Icon.X
              size={12}
        ...
        <div className="mt-2 md:mt-3 flex flex-col flex-1 px-1">
          <h3 className="text-[12px] sm:text-[13px] md:text-[15px] font-bold text-gray-200 line-clamp-1 group-hover/card:text-white transition-colors">
            {m.name}
          </h3>
    ...
    <div className="mb-8 md:mb-12 relative group/section">
      <div className="flex items-center justify-between mb-3 md:mb-4 px-1">
        <h2 className="text-[15px] sm:text-lg md:text-2xl font-black text-white uppercase tracking-tight flex items-center gap-2 md:gap-3">
          <span className="w-1 md:w-1.5 h-4 sm:h-5 md:h-7 bg-[#E50914] rounded-full"></span>{" "}
          {title}
        </h2>
        <button
          onClick={() => setView({ type: "list", slug, title, mode: type })}
          className="text-[#E50914] text-[9px] sm:text-[10px] md:text-xs font-bold hover:underline opacity-100 md:opacity-0 group-hover/section:opacity-100 transition-opacity"
        >
          XEM TẤT CẢ
        </button>
      </div>
      <div className="relative">
        <button
          onClick={() => scroll("left")}
          className="absolute left-0 top-[40%] -translate-y-1/2 z-[40] bg-black/60 hover:bg-black/90 text-white p-2 md:p-3 rounded-r-2xl backdrop-blur-md opacity-0 group-hover/section:opacity-100 transition-all hidden md:flex items-center shadow-2xl"
        >
          <Icon.ChevronLeft size={30} className="md:w-9 md:h-9" />
        </button>
        <div
          ref={scrollRef}
          className="flex gap-3 sm:gap-4 md:gap-6 overflow-x-auto scroll-smooth no-scrollbar pb-4 px-1 md:px-2"
        >
          {movies.map((m) => (
            <MovieCard
              key={m.slug}
        ...
        <button
          ...
          className="absolute right-0 top-[40%] -translate-y-1/2 z-[40] bg-black/60 hover:bg-black/90 text-white p-2 md:p-3 rounded-l-2xl backdrop-blur-md opacity-0 group-hover/section:opacity-100 transition-all hidden md:flex items-center shadow-2xl"
        >
    ...
    <>
      ...
      <header
        className={`fixed top-0 w-full z-[100] transition-all duration-300 ${
          scrolled
            ? "bg-[#050505]/95 backdrop-blur-md border-b border-white/5 py-2 md:py-3 shadow-2xl"
...
const HeroSlide = memo(({ movie, isActive, setView }) => {
  ...
  return (
    ...
    >
      <img
        src={backdropUrl}
        className="w-full h-full object-cover opacity-70 object-top md:object-center"
        alt={movie?.name}
        decoding="async"
      />
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-end items-start px-4 sm:px-6 md:px-12 max-w-7xl mx-auto h-full pb-16 sm:pb-20 md:pb-28">
        <div className="max-w-2xl w-full flex flex-col items-start text-left animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <div className="flex flex-wrap items-center justify-start gap-1.5 md:gap-2 text-[9px] sm:text-xs md:text-sm font-medium text-white/90 mb-4 md:mb-6 w-full">
            ...
            <span className="border border-white/30 px-1.5 py-0.5 rounded backdrop-blur-sm">
              {movie?.year || "2024"}
            </span>
            <span className="border border-white/30 px-1.5 py-0.5 rounded backdrop-blur-sm">
              Phần 1
          ...
          <button
            onClick={() => {
              setView({ type: "detail", slug: movie?.slug });
              window.scrollTo(0, 0);
            }}
  ...
  useEffect(() => {
    const fetchBannerData = async () => {
      try {
        ...
        const withBackdrops = tmdbResults.filter(
          (m) => m.tmdbData && m.tmdbData.backdrop_path
        );

        setBannerMovies(
          withBackdrops.length > 0
            ? withBackdrops.slice(0, 5)
            : tmdbResults.slice(0, 5)
        );
      } catch (error) {
        console.error(error);
      } finally {
        ...
        {bannerMovies.map((_, i) => (
          <button
            ...
            className={`rounded-full transition-all duration-500 ${
              i === currentIndex
                ? "w-5 h-1 md:w-8 md:h-1.5 bg-[#E50914]"
                : "w-1 h-1 md:w-1.5 md:h-1.5 bg-gray-500 hover:bg-gray-300"
            }`}
          />
        ))}
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-8 pt-20 md:pt-24 pb-10">
      <h2 className="text-[15px] sm:text-xl md:text-2xl font-black text-white mb-4 md:mb-6 uppercase tracking-tight flex items-center gap-2 md:gap-3">
        <span className="w-1 sm:w-1.5 h-4 sm:h-7 bg-[#E50914] rounded-full"></span>{" "}
        {title}
      </h2>
      ...
      {(loading || loadingMore) && (
        <div className="py-8 flex justify-center">
          <Icon.Loader2 className="animate-spin text-[#E50914]" size={28} />
        </div>
      )}
...
const MovieDetail = ({ slug, setView }) => {
  ...
  const backdropUrl = tmdbData?.backdrop_path
    ? `https://image.tmdb.org/t/p/original${tmdbData.backdrop_path}`
    : getImg(i?.poster_url || i?.thumb_url);
      ...
      <div className="relative min-h-[55vh] md:h-[80vh] w-full overflow-hidden flex flex-col justify-end pt-24 px-4 md:px-8">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto pb-6 md:pb-8 flex flex-col md:flex-row gap-5 md:gap-10 items-center md:items-end">
          ...
          <div className="flex-1 text-center md:text-left w-full">
            <h1 className="text-2xl sm:text-3xl md:text-6xl font-black text-white mb-1 md:mb-2 leading-tight drop-shadow-2xl">
              {i?.name}
            </h1>
            <p className="text-xs md:text-lg text-gray-400 font-bold mb-3 md:mb-4 drop-shadow-md">
              {i?.origin_name}
            </p>
            <div className="flex flex-wrap justify-center md:justify-start items-center gap-2 md:gap-3 mb-5 md:mb-6 text-gray-300 text-[10px] md:text-sm font-bold">
              ...
              <span>{i?.year}</span>
              <span>•</span>
              <span className="bg-[#E50914] px-1.5 py-0.5 rounded text-white text-[9px] md:text-[10px]">
                {i?.quality || "HD"}
              </span>
              <span>•</span>
              <span className="border border-gray-600 px-1.5 py-0.5 rounded">
                {i?.episode_current}
              </span>
            </div>
            <button
              ...
              className="w-full sm:w-fit md:min-w-[280px] bg-[#E50914] hover:bg-red-700 text-white px-8 py-3 md:px-10 md:py-3.5 rounded-full font-black flex justify-center items-center gap-2 md:gap-3 transition-all transform active:scale-95 shadow-xl uppercase tracking-widest text-[12px] md:text-base"
            >
              <Icon.Play
                size={16}
                fill="currentColor"
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mt-8 md:mt-12 grid md:grid-cols-12 gap-6 md:gap-8 items-start">
        ...
        <div className="md:col-span-4 bg-[#111] p-5 md:p-8 rounded-2xl border border-white/5 shadow-xl space-y-6 md:space-y-8 h-fit">
          {[
            ...
            <div
              key={x.l}
              className="space-y-1 md:space-y-1.5 border-b border-white/5 pb-3 md:pb-4 last:border-0 last:pb-0"
            >
              <p className="text-[9px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.2em]">
                {x.l}
              </p>
              <p className="text-[11px] md:text-sm font-bold text-gray-300 leading-snug">
                {x.v || "Đang cập nhật"}
              </p>
            </div>
          ))}
  ...
  return (
    <div className="pt-16 md:pt-28 pb-10 max-w-7xl mx-auto px-0 sm:px-4 md:px-8 animate-in fade-in duration-500">
      ...
      <div className="mt-6 md:mt-10 bg-[#111] p-4 md:p-6 rounded-none sm:rounded-2xl border-y sm:border border-white/5 shadow-2xl">
        <h1 className="text-lg md:text-2xl font-black text-white mb-1 md:mb-2">
          {data?.name}
        </h1>
        ...
        {data?.episodes?.map((s) => (
          <div key={s.server_name} className="mb-5 md:mb-6 last:mb-0">
            <p className="text-[10px] md:text-[11px] text-gray-500 font-black mb-3 md:mb-4 uppercase tracking-widest">
              {s.server_name}
            </p>
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2 md:gap-3">
              {s.server_data.map((e) => (
                <button
                  ...
                  className={`py-2 md:py-3 text-[11px] md:text-sm rounded-md font-bold transition-all ${
                    ep?.slug === e.slug
