  ...
  useEffect(() => {
    ...
    const delay = setTimeout(() => {
      fetch(`${API}/tim-kiem?keyword=${query}`, { signal: controller.signal })
        .then((r) => r.json())
        .then((data) => { setResults(data?.data?.items || []); setLoading(false); })
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex justify-center pt-16 md:pt-24 px-4">
      ...
      <div className="relative w-full max-w-3xl bg-[#111] rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[70vh] overflow-hidden">
        <form onSubmit={(e) => { e.preventDefault(); if (query) { setView({ type: "search", keyword: query }); onClose(); } }} className="flex items-center p-4 border-b border-white/5 bg-[#1a1a1a]">
          <Icon.Search className="text-gray-400 absolute left-6" size={20} />
          <input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Tìm kiếm phim..." className="w-full bg-transparent outline-none text-white pl-10 pr-10 py-2 text-base md:text-lg" />
        </form>
        <div className="overflow-y-auto flex-1 p-2 no-scrollbar">
          {loading ? (
            ...
            results.map((m) => (
              <div key={m.slug} onClick={() => { setView({ type: "detail", slug: m.slug }); onClose(); window.scrollTo(0, 0); }} className="flex gap-4 p-3 hover:bg-white/5 rounded-xl cursor-pointer transition border-b border-white/5 last:border-0 group/card">
                <SmartImage src={m.thumb_url} originName={m.origin_name} name={m.name} className="w-14 h-20 object-cover rounded-lg shadow-lg" alt={m.name} />
                <div className="flex flex-col justify-center">
                  <h4 className="text-sm md:text-base font-bold text-white line-clamp-1">{m.name}</h4>
                  <p className="text-[11px] md:text-xs text-gray-500 mt-1">{m.year} • {m.origin_name}</p>
                </div>
              </div>
            ))
          )}
...
const HeroSlide = memo(({ movie, isActive, setView }) => {
  ...
  return (
    <div className={`absolute inset-0 w-full h-full transition-opacity duration-1000 ease-in-out ${isActive ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
      <img src={backdropUrl} className="w-full h-full object-cover opacity-70 object-top md:object-center" alt={movie?.name} decoding="async" />
      ...
      {view.type === "home" ? (
        <>
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-8 relative z-20 pb-20 pt-4 md:pt-0">
            ...
            <MovieSection title="Phim Trending 🔥" slug="phim-bo" setView={setView} progressData={progressData} />
